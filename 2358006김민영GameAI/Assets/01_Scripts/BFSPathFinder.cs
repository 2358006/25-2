...
public class BFSPathFinder : MonoBehaviour
{
    ...
    GridManager gridManager = null; // 그리드 정보를 제공하는 매니저

    // Vector2Int 는 "(x, y) 좌표를 정수 단위로" 표현하는 타입
    // 미로는 타일 단위로 움직이기 때문에 소숮머 이하가 필요 없다 → 그래서 Vector2 대신 Vector2Int 자료형을 사용한다.
    // [SerializeField] 덕분에 Inspector에서 직접 시작/끝 좌표를 수정할 수 있어서, 알고리즘을 돌려보면서 여러 위치를 시험해 보기 적합하다.
    ...
    // 시작 지점에서 도착 지점까지의 최단 경로를 List<Vector2Int> 형태로 반환, 없으면 null 반환
    public List<Vector2Int> GetBFSPath()
    {
        ...
        // visited
        // - "이미 체크ㅐ본 타일 목록:
        // - 같은 곳을 여러 번 보지 않게 해서 무한 루프를 방지하고, 성능도 확보한다.
        HashSet<Vector2Int> visited = new HashSet<Vector2Int>();

        // 부모 추적 : 특정 노드(next)의 직전 위치(current)를 기록 → 경로 복원에 사용
        // cameFrom
        // - 예 (2, 1)은 (1,1)에서 왔다.
        // - 나중에 도착 지점에서 거꾸로 따라가며 경로를 복원할 때 사용한다.
        Dictionary<Vector2Int, Vector2Int> cameFrom = new Dictionary<Vector2Int, Vector2Int>();

        // 큐(Queue) : BFS 핵심 자료구조, FIFO 원칙으로 노드 탐색
        // queue
        // - BFS의 핵심인 큐(Queue)
        // - "먼저 들어온 좌표부터 꺼내서 처리하는 줄 서기 자료구조" (FIFO)
        // - 초기 설정 : queue.Enqueue(startLocation);
        //   → 시작 좌표를 제일 청므 탐색 대상으로 줄에 세운다.
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (Vector2Int direction in directions)
            {
                ...
                {
                    visited.Add(next);      // 큐에 추가
                    queue.Enqueue(next);    // 방문 처리
        ...
        // - while (queue.Count > 0) 이 루프가 끝났다는 것은, 더 이상 탐색할 좌표가 없는데도 도착지점에 도달하지 못했다는 것을 의미한다.
        // - 이때는 return null; 로 "갈 수 있는 길이 없다"는 결과를 돌려주도록 한다.

        return null;

    }

    // ReconstructPath : BFS 탐색 결과를 이용해서 " 실제 경로 리스트 " 를 만드는 메소드
    // 부모 추적 정보를 사용해 시작 지점에서 도착 지점까지의 경로를 복원하는 메소드
    // Dictionary로 부모 정보를 저장해둔 이유는.
    // - cameFrom Dictionary에는 Key : 자식 노드 좌표, Value : 부모 노드 좌표 형태로
